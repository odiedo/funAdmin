...
<body>
    ...
    <div class="col-md-8">
        <form id="manual-vet-form" class="text-center">
            <div class="mb-4">
                <label class="form-label mt-3">Please click on the school to edit:</label>
            </div>
    ...
    <div class="col-md-4 pt-4">
        <div class="card">
            <div class="card-body">
                ...
                <h5>School Categories</h5>
                <ul id="vetting-summary" class="list-group">
                    <li class="list-group-item d-flex justify-content-between"><span>Secondary Level:</span> <span id="issues-count">0</span></li>
                    <li class="list-group-item d-flex justify-content-between"><span>Tertiary Level:</span> <span id="affected-count">0</span></li>
                </ul>
            </div>
        </div>
    </div>                    
...
$(document).ready(function () {
    ...
    function fetchSchoolCounts() {
        $.ajax({
            url: 'analysis/school_count.php',
            type: 'GET',
                    ...
                    data.data.forEach((school, index) => {
                        schoolList += `
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                <input type="text" class="form-control school-name" data-index="${index}" value="${school.school}" readonly>
                                <span class="badge bg-light text-dark">${school.student_count}</span>
                                <button type="button" class="btn copy-btn" data-school="${school.school}"><i class="fas fa-copy"></i> </button>
                                <button type="button" class="btn edit-btn" data-index="${index}" data-school="${school.school}"><i class="fas fa-edit"></i></button>
                            </li>
                        `;
                    });
                    $('#school-list').html(schoolList);
                } else {
                    alert(data.message || 'An error occurred while fetching school data.');
                }
            },
            error: function () {
    ...
    // Enable inline editing of school names
    $(document).on('click', '.edit-btn', function (event) {
        event.preventDefault(); 

        const index = $(this).data('index');
        const schoolName = $(this).data('school');
        const input = $(`.school-name[data-index="${index}"]`);
        
        if (input.prop('readonly')) {
            input.prop('readonly', false);
            $(this).html('<i class="fas fa-save"></i>');
        } else {
            const newSchoolName = input.val();
            ...
            $.ajax({
                url: 'analysis/update_school.php',
                type: 'POST',
                data: {
                    old_school_name: schoolName,
    ...
    // Copy school name to clipboard
    $(document).on('click', '.copy-btn', function () {
        const schoolName = $(this).data('school');
        const tempInput = $('<input>');
        $('body').append(tempInput);
        tempInput.val(schoolName).select();
        document.execCommand('copy');
        tempInput.remove();
        alert(`Copied: ${schoolName}`);
    });
});
</script>
